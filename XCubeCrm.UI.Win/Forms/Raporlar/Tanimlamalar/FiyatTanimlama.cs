...
{
    public partial class FiyatTanimlama : BaseReportListForm
    {
        sqlBaglanti bgl = new sqlBaglanti();
        string araSorguCari = "", araSorguUrun = "", araSorguIsyeri = "", araSorguCari2 = "";
        public FiyatTanimlama()
        {
...
" when C.TRCODE = 13 and C.MODULENR = 6 THEN 'İşyerleri Arası İ.Bord. Müşteri Çeki' " +
...
" when C.TRCODE = 46 and C.MODULENR = 5 THEN 'Alınan Ser. Mes. Makbuzu' " +
" when C.TRCODE = 28 and C.MODULENR = 5 THEN 'Banka Alınan Hizmet Fat.' " +
...
" when B.TRCODE = 13 and B.MODULENR = 6 THEN 'İşyerleri Arası İ.Bord. Müşteri Çeki' " +
...
" when B.TRCODE = 46 and B.MODULENR = 5 THEN 'Alınan Ser. Mes. Makbuzu' " +
" when B.TRCODE = 28 and B.MODULENR = 5 THEN 'Banka Alınan Hizmet Fat.' " +
...
" WHEN CF.TRCODE = 32 then 'Perak.satış iade fat' " +
" WHEN CF.TRCODE = 33 then 'Topt. satış iade fat.' " +
" WHEN CF.TRCODE = 34 then 'Alınan hizmet fat' " +
" WHEN CF.TRCODE = 35 then 'Alınan proforma fat.' " +
" WHEN CF.TRCODE = 36 then 'Alım iade fat.' " +
" WHEN CF.TRCODE = 37 then 'Perakende satış fat.' " +
" WHEN CF.TRCODE = 38 then 'Toptan satış fat.' " +
" WHEN CF.TRCODE = 39 then 'Verilen hizmet faturası' " +
" WHEN CF.TRCODE = 40 then 'Verilen proforma fat.' " +
" WHEN CF.TRCODE = 41 then 'Verilen vade farkı fat.' " +
" WHEN CF.TRCODE = 42 then 'Alınan Vade farkı fat.' " +
" WHEN CF.TRCODE = 43 then 'Alınan fiyat farkı fat.' " +
" WHEN CF.TRCODE = 44 then 'Verilen fiyat farkı fat.' " +
...
" WHEN STL.TRCODE IN(4) and LINETYPE IN(4) then 'Alınan Hizmet' " +
" WHEN STL.TRCODE IN(3)AND LINETYPE IN(2) then 'Alış İ.İndirim' " +
" WHEN STL.TRCODE IN(9)AND LINETYPE IN(4) then 'Verilen Hizmet F.' " +
...
"   ,SUM(CASE WHEN STL.TRCODE IN(7, 8, 1, 26, 9)AND STL.LINETYPE IN(3) THEN(STL.TOTAL) " +
...
"   ,SUM(CASE WHEN STL.TRCODE IN(7, 8, 1, 26, 9)AND STL.LINETYPE IN(0, 4) THEN(STL.TOTAL / 1) " +
...
"   ,MAX(PYP.CODE) VADE " +
"   ,SUM(CASE WHEN STL.TRCODE IN(7, 8, 1, 26)  THEN(CASE WHEN STL.LINETYPE IN(1, 2) THEN  STL.TOTAL ELSE 0 END) " +
